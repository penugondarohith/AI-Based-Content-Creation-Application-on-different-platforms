...
import { clearGuestWorkspace } from "@/services/project-service";
...
export class LocalAuthService implements AuthService {
  ...
  continueAsGuest() { const previous = getStoredSession(); if (previous?.accountType === "GUEST") clearGuestWorkspace(previous.id); const user: User = { id: `guest_${Date.now()}`, name: "Guest User", email: "guest@local.contentforge", age: 0, accountType: "GUEST", createdAt: new Date().toISOString() }; saveStoredSession(user); return user; }
  ...
  signOut() { const session = getStoredSession(); if (session?.accountType === "GUEST") clearGuestWorkspace(session.id); clearStoredSession(); }
