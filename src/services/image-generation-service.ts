...
export async function generateImageForContent(
  ...
  const context = buildVisualContext(input);
  const prompt = buildImagePrompt(
    {
      ...context,
      visualDirection: {
        ...context.visualDirection,
        concept: `${context.visualDirection.concept}, ${getImageVariation(variation)}`,
      },
    },
    aspectRatio,
    visualStyle,
  ...
  return imageGenerationProvider.generateImage({
    projectId: input.project.id,
    generatedContentId: input.project.generatedContent.find((item) => item.calendarPostId === input.calendarPost.id)?.id ?? `generated-${input.calendarPost.id}`,
    calendarPostId: input.calendarPost.id,
    prompt,
  }, variation);
}