...
export function ContentStudioWorkspace() {
  const { industryId, configuration, referenceFiles, brandContext, categoryDetails, contentStrategy, generatedContent, upsertGeneratedContent, setProjectStatus } = useProject();
  ...
  const project = useMemo<Project>(() => ({ id: "project_current", userId: "current-user", name: `${industry.name} Campaign`, industry: industryId, configuration, referenceFiles, brandContext, categoryDetails, contentStrategy, generatedContent, status: "STRATEGY_READY", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }), [industry.name, industryId, configuration, referenceFiles, brandContext, categoryDetails, contentStrategy, generatedContent]);
  ...
  const generateOne = async (post = selectedPost, variation = 0) => {
    ...
    try {
      const input = { project, calendarPost: post, industryDNA: industry, brandContext, categoryDetails, contentStrategy };
  ...
  return <div className="space-y-5"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-primary">Content studio</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.04em]">Generate, refine, and validate</h1><p className="mt-2 text-sm text-muted">Strategy-aligned written content for {industry.name}.</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={generating} onClick={() => void generateAll()} className="rounded-lg bg-primary px-3 py-2 text-xs font-medium text-white hover:bg-primary-strong disabled:opacity-60">{generating ? "Generating content..." : "Generate campaign content"}</button><Link href="/create/strategy" className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-xs text-muted hover:bg-surface-muted"><ArrowLeft size={14} /> Strategy</Link><Link href="/create/review" className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-xs text-muted hover:bg-surface-muted">Content review <ArrowRight size={14} /></Link></div></div>{progress && <GenerationProgress completed={progress.completed} total={posts.length} current={progress.current} /> }<div className="grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)_280px]"><CampaignPostSidebar posts={posts} generated={generatedContent} selectedId={selectedPost?.id ?? ""} onSelect={(id) => { setSelectedId(id); setDraft(null); }} /><main className="min-w-0 space-y-5">{selectedItem ? <><ContentEditor item={selectedItem} onChange={updateDraft} onSave={saveDraft} onRegenerate={(mode) => { const modes = ["professional", "creative", "engaging", "shorter", "detailed", "different-angle"]; void generateOne(selectedPost, (modes.indexOf(mode) + selectedItem.postNumber + 1) % 6); }} /><ContentPreview item={draft ?? selectedItem} /></> : <EmptyContentState onGenerateFirst={() => void generateOne()} onGenerateAll={() => void generateAll()} />}</main>{selectedItem && <GenerationIntelligenceSidebar item={selectedItem} onRecheck={() => setDraft({ ...selectedItem, qualityReport: checkContentQuality(selectedItem.content, selectedItem.generationContext) })} />}</div></div>;
}
